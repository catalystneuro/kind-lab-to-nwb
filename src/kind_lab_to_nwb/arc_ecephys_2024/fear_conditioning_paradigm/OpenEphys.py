...
def loadFolderToArray(folderpath, channels='all', chprefix='CH',
    ...
    if session == '0':
        if chprefix == 'CH':
            # filelist = [source + '_RhythmData_' + chprefix + x + '.continuous' for x in map(str, channels)]
            filelist = [str(int(float(source))) + '_' + chprefix + x + '.continuous' for x in map(str, channels)]
        elif chprefix == 'AUX':
            # filelist = [source + '_RhythmData_' + chprefix + x + '.continuous' for x in map(str, channels)]
            filelist = [i for i in os.listdir(folderpath) if chprefix in i]
    else:
        filelist = [str(int(float(source))) + '_' + chprefix + x + '_' + session + '.continuous' for x in
                    map(str, channels)]

    t0 = time.time()
    numFiles = 1

    channel_1_data = loadContinuous(os.path.join(folderpath, filelist[0]), dtype)
    ...
    for i, f in enumerate(filelist[1:]):
        data_continuous = loadContinuous(os.path.join(folderpath, f), dtype)
...
def _get_sorted_channels(folderpath, chprefix='CH', session='0', source='100'):
    if chprefix == 'CH':
        ...
        Files = [f for f in os.listdir(folderpath) if
                 '.continuous' in f and chprefix in f and str(int(float(source))) in f]
    ...
    if session == '0':
        ...
        if chprefix == 'CH':
            ...
            Chs = sorted([int(f.split(chprefix)[1].split('.')[0]) for f in Files])
    else:
        Files = [f for f in Files if len(f.split('_')) == 3
